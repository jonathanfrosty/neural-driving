...
let keyP;
let keyText = "Edit Grid: G | Set Spawn: S | Respawn: SPACE | Training Mode: T | Run Neural Net: N | Toggle Lasers: L | Save Course: ENTER (max 5) | Load Course: 1-5 | Clear Course: ESC";

function setup() {
    createCanvas(windowWidth, windowHeight);
    grid = new Grid(gridSquaresAcross);
    car = new Car(maxSpeed, turningAbility, carLaserCount, minCarLaserLength, maxCarLaserLength);
    newTrainingData = defaultTrainingData;

    modeP = createP().position(30, 5).html("Mode: " + modeText);
    keyP = createP().position(30, height - 25).html(keyText);
}
...
function draw() {
    ...
    if(!gridEditingMode && !car.crashed) {
        ...
        if(trainingMode) {
            if(approximateTrainingTimeElapsed == 0) console.log("started training mode...");
                ...
                console.log((approximateTrainingTimeElapsed / 60) + " seconds elapsed (" + newTrainingData.length + " training elements)");
            }
...
function keyPressed() {
    ...
    if(keyCode == 32) car = new Car(maxSpeed, turningAbility, carLaserCount, minCarLaserLength, maxCarLaserLength); // SPACE
    ...
    if(keyCode == 84 && !trainingNeuralNet && !gridEditingMode) { // T
    ...
    if(keyCode == 78 && neuralNet != null) { // N      
...
function updateTrainingData() {
    ...
    let data = { input: sensorDistances, output: [isMovingForward, isMovingLeft, isMovingRight]};
...
function trainNeuralNetwork() {
    ...
    neuralNet.train(trainingData, { log: true, logPeriod: 10, errorThresh: errorThresh, learningRate: learningRate, iterations: iterations });
